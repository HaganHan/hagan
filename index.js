const hagan = {//fnHideDom fnShowDom
  ...
  fnSetHtmlFontSize() {
      ...
      This.fnOnReSize(function () {
          ...
          nBodyWidth = This.fnGetBodyWidth();

          This.jInfo.rem = nBodyWidth / 10;
  ...
  fnSetMobileStyle(eElement, sStyle, nPx, designWidth = 750) {
      eElement.style[sStyle] = (this.jInfo.rem * 10 * nPx / designWidth) + "px";
  },
  ...
  //得到html高度  hagan.fnGetHtmlHeight();  !IE8
  fnGetHtmlHeight() {
  ...
  //得到body宽度  hagan.fnGetHtmlHeight();  !IE8
  fnGetBodyWidth(){
  ...
  //得到body高度  hagan.fnGetHtmlHeight();  !IE8
  fnGetBodyHeight(){
  ...
  fnHideDom(eElement, nTime = 500, fnCallBack = Function){

      const domInfo=this.jInfo.hideDomInfo[eElement.id]=this.jInfo.hideDomInfo[eElement.id]||{};

      if (domInfo.lock){
          return console.warn(`${eElement.id}正在show or hide，请show or hide结束后再调用hide方法`);
      }

      domInfo.lock=true;

      domInfo.opacity=this.fnGetFinalStyle(eElement,"opacity"); //获取对应dom的opacity初始值并储存到hagan.jInfo.hideDomInfo对象上
      eElement.style.opacity=domInfo.opacity; //设置对应dom的opacity初始值

      domInfo.display=this.fnGetFinalStyle(eElement,"display"); //储存对应dom的display初始值到hagan对象上
      ...
      const hide=(ms)=>{
          setTimeout(()=>{
              eElement.style.opacity =Number(eElement.style.opacity)-item;
              if (eElement.style.opacity>0){
                  hide(nTime/(times-1));
              }else {
      ...
      // `;
      //
      // this.fnHideDom = function (eElement, nTime = 500, fnCallBack = Function) {
      //
      //     eElement.classList.remove("a-ShowDom");
      //     eElement.classList.add("a-HideDom");
      //     setTimeout(function () {
      //         eElement.classList.add("z-none");
      //         fnCallBack();
      //     }, nTime);
      //
      // };
      // this.fnHideDom(eElement, nTime, fnCallBack);

  },

  fnShowDom(eElement, nTime = 500, fnCallBack = Function){

      const domInfo=this.jInfo.hideDomInfo[eElement.id];

      if (domInfo.lock){
          return console.warn(`${eElement.id}正在show or hide，请show or hide结束后再调用show方法`);
      }

      domInfo.lock=true;

      const targetOpacity=domInfo.opacity;
      eElement.style.display=domInfo.display;
      ...
      const show=(ms)=>{
          setTimeout(()=>{
              eElement.style.opacity =Number(eElement.style.opacity)+item;
              if (eElement.style.opacity<targetOpacity){
                  show(nTime/(times-1));
              }else {
                  eElement.style.opacity=targetOpacity;
                  domInfo.lock=false;
              }
          },ms);
      };
      ...
      // `;
      //
      // this.fnShowDom = function (eElement, nTime = 500, fnCallBack = Function) {
      //
      //     eElement.classList.remove("z-none");
      //     eElement.classList.remove("a-HideDom");
      //     eElement.classList.add("a-ShowDom");
      //     setTimeout(function () {
      //         fnCallBack();
      //     }, nTime);
      //
      // };
      // this.fnShowDom(eElement, nTime, fnCallBack);
  ...
  fnIsVerticalScreen(){
      ...
      if(this.fnGetWindowWidth()>this.fnGetWindowHeight()){
          bResult=false;
      }else {
          bResult=true;
      }
